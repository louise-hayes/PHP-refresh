...
class RegionDashboard {
    ...
    /**
     * Function for to show graph and pie chart of Region
     * @param type $data
     * @return int|string
     */
    public function regionDashboard($data) {
        ...
        $manvalues = array($data['regionId'], $data['startDate'], $data['endDate'], $data['analyticsType']);
        ...
        if ($checkdata === 0) {
            ...
            $bindparams = array($data['regionId'], $data['startDate'] . " 00:00:00", $data['endDate'] . " 23:59:59");
            ...
            if (in_array("1", explode(',',$data['analyticsType']))){
                $sql = "SELECT coe_users.first_name as escalation_first_name, coe_users.last_name as escalation_last_name, 
                    ...
                    coe_issue_status.escalation_manager_id, coe_escalation_request.country_id,
                    ...
                    WHERE coe_escalation_request.country_id=? && coe_escalation_request.escalation_type_id=1 && coe_escalation_request.insert_date BETWEEN ? AND ?";
            ...
            if (in_array("2", explode(',',$data['analyticsType']))){
                $sql1 = "SELECT coe_users.first_name as escalation_first_name, coe_users.last_name as escalation_last_name, 
                    ...
                    WHERE coe_escalation_request.country_id=? && coe_escalation_request.escalation_type_id=2 && coe_escalation_request.insert_date BETWEEN ? AND ?";
            ...
            if (in_array("4", explode(',',$data['analyticsType']))){
                $sql2 = "SELECT coe_users.first_name as escalation_first_name, coe_users.last_name as escalation_last_name, 
                        ...
                        coe_issue_status.escalation_manager_id, coe_escalation_request.country_id, coe_escalation_request.escalation_sub_type_id
                        FROM `coe_escalation_request`
                        LEFT JOIN coe_protocol_numbers ON coe_escalation_request.protocol_id=coe_protocol_numbers.protocol_id
                        LEFT JOIN coe_sitenames ON coe_escalation_request.sitename=coe_sitenames.sitename_id
                        LEFT JOIN coe_countries ON coe_escalation_request.country_id=coe_countries.country_id
                        ...
                        LEFT JOIN coe_users ON coe_issue_status.escalation_manager_id=coe_users.user_id
                        LEFT JOIN coe_users as legal ON coe_issue_status.legal_user_id=legal.user_id
                        WHERE coe_escalation_request.country_id=? && coe_escalation_request.escalation_type_id=4 && coe_escalation_request.insert_date BETWEEN ? AND ?";
            ...
            foreach ($arrayCombine['data'] as $key => $value) {
                $arraySortDesc1[$value['country_id']][] = $value;
            }
            krsort($arraySortDesc1, SORT_ASC);

            //print_r($arraySortDesc1);
            ...
            foreach ($arraySortDesc2 as $key => $value) {
                ...
                $legal[] = array('below7' => $below7, 'above7' => $above7, 'above12' => $above12, 'pending' => $pending, 'languageIssues' => $languageIssues, 'budgetIssues' => $budgetIssues, 'icfIssues' => $icfIssues, 'issues_count' => count($value), 'legal_id' => $value['0']['legal_user_id'], 'legal_first_name' => $value['0']['legal_first_name'], 'legal_last_name' => $value['0']['legal_last_name'], 'barChart'=>$requesType);
