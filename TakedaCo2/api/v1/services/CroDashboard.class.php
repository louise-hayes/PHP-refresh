...
class CroDashboard {
    ...
    /**
     * Function for to show graph and pie chart of CRO
     * @param type $data
     * @return int|string
     */
    public function croDashboard($data) {
        ...
        if (array_key_exists('authorization', $headers)) {
            ...
            try {
                ...
                if ($data1->json_data->exp >= time()) {
                    ...
                    $manvalues = array($data['croId'], $data['startDate'], $data['endDate'], $data['analyticsType']);
                    ...
                    if ($checkdata === 0) {
                        ...
                        $bindparams = array($data['croId'], $data['startDate'] . " 00:00:00", $data['endDate'] . " 23:59:59");
                    ...
                    coe_issue_status.escalation_manager_id, coe_escalation_request.country_id,
                    coe_escalation_request.escalation_sub_type_id
                    FROM `coe_escalation_request`
                    LEFT JOIN coe_protocol_numbers ON coe_escalation_request.protocol_id=coe_protocol_numbers.protocol_id
                    LEFT JOIN coe_sitenames ON coe_escalation_request.sitename=coe_sitenames.sitename_id
                    LEFT JOIN coe_countries ON coe_escalation_request.country_id=coe_countries.country_id
                    ...
                    WHERE coe_escalation_request.user_id=? && coe_escalation_request.escalation_type_id=4 && coe_escalation_request.insert_date BETWEEN ? AND ?";
                        ...
                        //multiple arrays merge into one array
                        ...
                        //sorting the array according to key values
                        krsort($arraySortDesc, SORT_ASC);

                        $arraySortDesc1 = array();
                        foreach ($arrayCombine['data'] as $key => $value) {
                            $arraySortDesc1[$value['user_id']][] = $value;
                        }
                        ...
                        //check if no record found return the response message
                        ...
                        foreach ($arraySortDesc1 as $key => $value) {
                            ...
                            foreach ($value as $key1 => $value1) {
                                ...
                                //adding budget count according to the escalation type
                        ...
                        $pieChart[] = array('languageIssues' => $languageIssues, 'budgetIssues' => $budgetIssues, 'issues_count' => count($value), 'pie_chart' => $requesType);
